...
\section{Choosing a unit of analysis}\label{choosing-a-unit-of-analysis}}
...
\hypertarget{administrative--window-exceptional-clearance-segment}{%
\chapter{Administrative- Window Exceptional Clearance Segment}\label{administrative--window-exceptional-clearance-segment}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{load}\NormalTok{(}\StringTok{"data/nibrs\_offense\_segment\_segment\_2019.rda"}\NormalTok{)}
\NormalTok{offense }\OtherTok{\textless{}{-}}\NormalTok{ nibrs\_offense\_segment\_segment\_2019}
\FunctionTok{rm}\NormalTok{(nibrs\_offense\_segment\_segment\_2019)}
\FunctionTok{gc}\NormalTok{()}
\CommentTok{\#\textgreater{}             used   (Mb) gc trigger   (Mb)  max used   (Mb)}
\CommentTok{\#\textgreater{} Ncells   5081061  271.4    9818648  524.4   5098871  272.4}
\CommentTok{\#\textgreater{} Vcells 192420688 1468.1  268965598 2052.1 192464051 1468.4}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{drug-or-alcohol-use}{%
\subsection{Drug or alcohol use}\label{drug-or-alcohol-use}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{load}\NormalTok{(}\StringTok{"data/nibrs\_offender\_segment\_segment\_2019.rda"}\NormalTok{)}
\NormalTok{offender }\OtherTok{\textless{}{-}}\NormalTok{ nibrs\_offender\_segment\_segment\_2019}
\FunctionTok{rm}\NormalTok{(nibrs\_offender\_segment\_segment\_2019)}
\FunctionTok{gc}\NormalTok{()}
\CommentTok{\#\textgreater{}            used  (Mb) gc trigger  (Mb) max used  (Mb)}
\CommentTok{\#\textgreater{} Ncells  5081047 271.4    9819981 524.5  5098857 272.4}
\CommentTok{\#\textgreater{} Vcells 88748472 677.1  108845140 830.5 88791835 677.5}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{offender}\SpecialCharTok{$}\NormalTok{age\_of\_offender[offender}\SpecialCharTok{$}\NormalTok{age\_of\_offender }\SpecialCharTok{\%in\%} \StringTok{"over 98 years old"}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{99}
\NormalTok{offender}\SpecialCharTok{$}\NormalTok{age\_of\_offender[offender}\SpecialCharTok{$}\NormalTok{age\_of\_offender }\SpecialCharTok{\%in\%} \StringTok{"unknown"}\NormalTok{] }\OtherTok{\textless{}{-}} \ConstantTok{NA}
\NormalTok{offender}\SpecialCharTok{$}\NormalTok{age\_of\_offender }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(offender}\SpecialCharTok{$}\NormalTok{age\_of\_offender)}
\FunctionTok{make\_stat\_count\_plots}\NormalTok{(offender, }\StringTok{"age\_of\_offender"}\NormalTok{, }\AttributeTok{count =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"\% of Offenders"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Age"}\NormalTok{)}
\CommentTok{\#\textgreater{} Warning: Removed 2943237 rows containing non{-}finite values (stat\_count).}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.9\linewidth]{nibrsbook_files/figure-latex/unnamed-chunk-2-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{offender}\SpecialCharTok{$}\NormalTok{sex\_of\_offender[offender}\SpecialCharTok{$}\NormalTok{sex\_of\_offender }\SpecialCharTok{==} \StringTok{"m"}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{"male"}
\NormalTok{offender}\SpecialCharTok{$}\NormalTok{sex\_of\_offender[}\FunctionTok{is.na}\NormalTok{(offender}\SpecialCharTok{$}\NormalTok{sex\_of\_offender)] }\OtherTok{\textless{}{-}} \StringTok{"unknown"}
\NormalTok{offender }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{sex\_of\_offender =} \FunctionTok{capitalize\_words}\NormalTok{(sex\_of\_offender)) }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{crimeutils}\SpecialCharTok{::}\FunctionTok{make\_barplots}\NormalTok{(}\StringTok{"sex\_of\_offender"}\NormalTok{, }\AttributeTok{count =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"\% of Offenders"}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\NormalTok{percent)}
\CommentTok{\#\textgreater{} Scale for \textquotesingle{}y\textquotesingle{} is already present. Adding another scale for \textquotesingle{}y\textquotesingle{}, which will}
\CommentTok{\#\textgreater{} replace the existing scale.}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.9\linewidth]{nibrsbook_files/figure-latex/unnamed-chunk-3-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{offender}\SpecialCharTok{$}\NormalTok{race\_of\_offender[}\FunctionTok{is.na}\NormalTok{(offender}\SpecialCharTok{$}\NormalTok{race\_of\_offender)] }\OtherTok{\textless{}{-}} \StringTok{"unknown"}
\NormalTok{offender }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{race\_of\_offender =} \FunctionTok{capitalize\_words}\NormalTok{(race\_of\_offender)) }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{crimeutils}\SpecialCharTok{::}\FunctionTok{make\_barplots}\NormalTok{(}\StringTok{"race\_of\_offender"}\NormalTok{, }\AttributeTok{count =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"\% of Offenders"}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\NormalTok{percent)}
\CommentTok{\#\textgreater{} Scale for \textquotesingle{}y\textquotesingle{} is already present. Adding another scale for \textquotesingle{}y\textquotesingle{}, which will}
\CommentTok{\#\textgreater{} replace the existing scale.}
\end{Highlighting}
...
\begin{center}\includegraphics[width=0.9\linewidth]{nibrsbook_files/figure-latex/unnamed-chunk-4-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{nibrsbook_files/figure-latex/unnamed-chunk-5-1} \end{center}

\hypertarget{ethnicity}{%
\subsection{Ethnicity}\label{ethnicity}}

\begin{center}\includegraphics[width=0.9\linewidth]{nibrsbook_files/figure-latex/unnamed-chunk-6-1} \end{center}

\hypertarget{homicide-type}{%
\subsection{Homicide type}\label{homicide-type}}

\hypertarget{arrestee-and-window-arrestee-segment}{%
\chapter{Arrestee and Window Arrestee Segment}\label{arrestee-and-window-arrestee-segment}}
...
\hypertarget{group-b-arrest-reports-segment}{%
\chapter{Group B Arrest Reports Segment}\label{group-b-arrest-reports-segment}}
